...
def safe_productivity_score(tasks_completed, hours_worked):
    try:
        ...
        if hours_worked<=0:
            return "Hours worked should be greater than 0"
    except ValueError:
        return "Hours worked must be a valid number"
    try:
        tasks_completed=float(tasks_completed)
        if tasks_completed<0:
            return "task completed cannot be neagative"
    except ValueError:
        return "Task completed must be a valid number"
    score=tasks_completed/hours_worked
    return f" Score= {score:.2f}"
tasks=input("enter no of tasks completed: ")
hours=input("enter no of hours completed: ")
